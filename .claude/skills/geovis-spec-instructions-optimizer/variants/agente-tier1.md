...
Everything else the consumer already rejects by name before rendering: dangling `sourceId`/`mapDataId`, a layer carrying both `mapDataId` and `propertyName`, a `mapType` that joins no declared source, point/symbol layers on polygon sources, geometry inside `mapData`, non-ascending thresholds, inverted `sizeBy` ranges, a painted variable with no legend, and any property the schema does not declare. Getting them right saves a round trip; restating them to yourself does not.