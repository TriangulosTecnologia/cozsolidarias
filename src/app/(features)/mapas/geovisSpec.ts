...
import {
  ...
  ASSENTAMENTOS_MAP_DATA_ID,
  ASSENTAMENTOS_SOURCE,
  ASSENTAMENTOS_SOURCE_ID,
  buildAssentamentosLayer,
  buildAssentamentosOutlineLayer,
  buildEstadosFillLayer,
  ESTADOS_SOURCE,
  toAssentamentoStatusRows,
} from './geovisAssentamentos';
import {
  buildCafHexbinLayer,
...
/** Re-exported so consumers keep importing the settlement shape from here. */
...
type MapOverlays = {
  ...
  /** The H3 hexagon grid with its CAF counts; painted in `cafs-hexbin` mode. */
...
const buildSources = ({
  ...
  if (mode === 'cafs-hexbin') {
    return [...SOURCES, buildCafHexbinSource(overlays.cafHexbin)];
  }
  return SOURCES;
};
...
const buildOverlayLayers = ({
  ...
  if (mode === 'assentamentos') {
    ...
    // Hidden under the hexagon grid, which covers the country: drawing ~5.5k
    // município polygons beneath an opaque layer is wasted work, and — the part
    // that actually breaks — geovis tracks hover on EVERY polygon layer with an
    // `activeLegendId`, so a visible fill under the grid would register a second
    // mousemove handler and the two would race for the tooltip. An invisible
    // layer returns no feature from `queryRenderedFeatures`, so its handler
    // never fires.
    //
    // Hidden, NOT dropped: the `cozinhas-por-municipio` join points at this
    // layer's source, and removing it strands that reference and makes geovis
    // refuse the whole spec.
    layers.push({
      ...buildFillLayer(mode, hoverTooltipRender),
      ...(mode === 'cafs-hexbin' ? { visible: false } : {}),
    });
  }
  ...
  // The hexagon grid: one joined layer, added over the fill and under the
  // kitchen overlays. It covers the whole country at 0.85 opacity, so the fill
...
const buildMapData = ({
  ...
  // The hexagon grid's own join. Its `joinKey` is what promotes each cell's H3
  // index to the MapLibre feature id — a string, which the feature's top-level
  // `id` could not carry.
  if (mode === 'cafs-hexbin') {
    data.push(...buildCafHexbinMapData(overlays.cafHexbin));
  }
