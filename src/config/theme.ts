...
const chakraConfig = defineConfig({
  ...
  theme: {
    ...
    keyframes: {
      ...
      /**
       * Breathing pulse (scale + opacity) — used by the map loading indicator.
       * Combines a gentle grow with a fade so the mark reads as "breathing"
       * rather than zooming.
       */
      pulseScale: {
...
/**
 * Chakra UI design system — the single source of truth for the app's visual
 * language. Pass this to `<ChakraProvider value={system}>` in `providers.tsx`.
 *
 * @example
 ...
 * const fill = mapTokens.dataviz.color.sequential[1][4];
 */
